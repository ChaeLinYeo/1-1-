...
def trinum(n):
    if n >= 1:
        return n*(n+1)/2-k+n # 여기에 알맞은 코드를 채워 넣자.
    else:
...
def is_pal(s):
    return len(s) <=1 or s[0] == s[-1] and is_pal(s[1:-1]) # 여기에 알맞은 코드를 채워 넣자.
...
# 재귀함수
def remove(x,xs):# 여기에 알맞는 코드를 채워 넣자.
    x.remove(xs)
# 테스트 케이스
...
def permutation0(n,k):
    if k > 0:
        return (n-k+1)*(n-k+2)/2 # 여기에 적절한 식을 채워 넣자.
    else:
        return 0
...
def drop_before(s,index):
    a=[]
    while s !=[] and index >0: # while 문 버전
        s=s[1:]
        index=index-1
    return a
